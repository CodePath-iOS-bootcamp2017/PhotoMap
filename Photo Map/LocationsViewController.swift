...
class LocationsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    let CLIENT_ID = "AR3XVXPF03MVJZ2A44KG3ZB2O2U42FLRZ4SGJQEJONFDT5AC"
    let CLIENT_SECRET = "1IMMGYNNQ5BANMQ21RH0QDL3GFU3LFW0QYCVYRZISMG2ITQP"
            ...
            completionHandler: { (dataOrNil, response, error) in
                if let data = dataOrNil {
                    if let responseDictionary = try! JSONSerialization.jsonObject(
                        ...
                        if let results = responseDictionary.value(forKeyPath: "response.venues") as? NSArray {
